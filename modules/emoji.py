...
async def handle_checkmark_reaction(bot, payload, original_poster_id):
    ...
    try:
        ...
        if interaction.component.label == "Yes":
            await interaction.edit_message(content="Excellent! We're pleased to know you're satisfied. This thread will now be closed.")
            if thread:
                await thread.delete()
        else:
            await interaction.edit_message(content="We're sorry to hear that. We'll strive to do better.")
