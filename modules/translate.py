    ...
    async def show_translate_modal(self, inter, message=None, author=None):
        ...
        try:
            ...
            if chat_response and chat_response.response:
                ...
                if author:
                    original_field_name += f" (by {author})"
                embed.add_field(name=original_field_name, value=f"```{message}```", inline=False)
                ...
                await modal_inter.response.send_message("❌ There was an error processing the translation.")
        except asyncio.TimeoutError:
            await inter.followup.send("⏰ The translation request timed out.")
